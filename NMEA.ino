...
int iterator = 0;
const int nLECTURAS = 5; // this is for bearingToTarget calculation purpouse
float bearingToTarget = 0;
// NeoGPS::Location_t target(258893020L, -803546210L); // North Miami
NeoGPS::Location_t target(259189500L, -801822730L); // North Miami
// NeoGPS::Location_t target(258354873L, -801918920L); // 90 degrees
static void doSomeWork()
{
  if (fix.valid.location && fix.valid.date && fix.valid.time)
  {
    float dist = fix.location.DistanceMiles(target);
    Serial.print(dist);
    Serial.println(F(" miles"));
    ...
    if (dist != 0)
    {
      ...
      Serial.println(F(" bearingToTarget"));
    }
    else
    {
...
// Average Angle Calculator
static void AngleCalculator(float bearing)
{
  bearingToTarget += bearing;
  iterator++;
  if (iterator == nLECTURAS)
  {
    bearingToTarget /= nLECTURAS;
  }
}

// Fixing the path
void moveTo(float angleToFix)
{
  Serial.print(angleToFix);
  Serial.println(F(" angleToFix"));
}
